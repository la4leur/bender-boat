...
import { Anchor, Users, Navigation, Clock, ArrowRightLeft, DollarSign, Search } from 'lucide-react';
        ...
        <div className="flex overflow-x-auto px-2 gap-1">
          {tabs.map(tab => (
            ...
            >
              {tab.icon}
              {tab.label}
              {tab.badge && <span className={`badge badge-xs ${tab.id === 'changes' || tab.id === 'travel' ? 'badge-accent' : 'badge-secondary'}`}>{tab.badge}</span>}
            </button>
          ))}
        </div>
      </div>
